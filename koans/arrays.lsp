...
(define-test test-make-your-own-array
  ...
  (let ((color-cube nil)
	(colors (make-array '(3) :initial-contents '(:red :blue :white))))
	...
	(dotimes (z 3)
	  (if (evenp (+ y z))
	      ...
	      (setf (aref color-cube x y z) (aref colors x))))))
...
(define-test test-adjustable-array
  ...
  (let ((x (make-array '(2 2) :initial-element 5 :adjustable t)))
    ...
    (adjust-array x '(3 4) :initial-element 4)
    (assert-equal (array-dimensions x) '(3 4))
    (assert-equal (aref x 2 3) 4)))
